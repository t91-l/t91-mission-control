...
export default function Home() {
  ...
  const { activeTab, setActiveTab, setCurrentUser, setDashboardMode, setGatewayAvailable, setLocalSessionsAvailable, setCapabilitiesChecked, setSubscription, setDefaultOrgName, setUpdateAvailable, setOpenclawUpdate, showOnboarding, liveFeedOpen, toggleLiveFeed, showProjectManagerModal, setShowProjectManagerModal, fetchProjects, setChatPanelOpen, bootComplete, setBootComplete, setAgents, setSessions, setProjects, setInterfaceMode, setMemoryGraphAgents, setSkillsData } = useMissionControl()

  // Sync URL → Zustand activeTab
  const pathname = usePathname()
  const panelFromUrl = pathname === '/' ? 'overview' : pathname.slice(1)
  const normalizedPanel = panelFromUrl === 'sessions' || panelFromUrl === 'chat' || panelFromUrl === 'office'
    ? 'overview'
    : panelFromUrl
  ...
  useEffect(() => {
    ...
    if (panelFromUrl === 'sessions' || panelFromUrl === 'chat' || panelFromUrl === 'office') {
  ...
  useEffect(() => {
    ...
    // Keep onboarding available from Settings, but never block the main dashboard
    // automatically on operator deployments.
    markStep('config')
...
const ESSENTIAL_PANELS = new Set([
  'overview', 'agents', 'tasks', 'activity', 'logs', 'settings',
])
